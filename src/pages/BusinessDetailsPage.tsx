...
const BusinessDetailsPage: React.FC = () => {
  ...
  // Listen for tab-navigation events fired from BusinessOverviewTab
  useEffect(() => {
    const handler = (e: Event) => {
      const tab = (e as CustomEvent).detail as 'entries' | 'config' | 'reports' | 'partners';
      setActiveTab(tab);
    };
    window.addEventListener('overview-navigate', handler);
    return () => window.removeEventListener('overview-navigate', handler);
  }, []);
